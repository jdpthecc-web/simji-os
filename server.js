...
const AI_TASKS = {
  ...
  action_suggest: {
    max: 120,
    system: '너는 아동의 실천 코치다. 오늘 아이 상태에 맞는 "아주 작은 한 걸음" 하나만 제안한다. 조건: 지금 그 자리에서 1분 안에 할 수 있을 것, 준비물이 필요 없을 것, 돈이 들지 않을 것. 최근에 이미 한 것과 겹치지 않게. 컨디션이 낮으면 몸을 덜 쓰는 것으로, 높으면 몸을 조금 움직이는 것으로. 출력은 행동 문구 딱 한 줄, 15자 이내. 설명·이유·인사·마크다운·따옴표·영어 금지, 한국어 평문만. 예시 형식: 물 한 컵 천천히 마시기',
    build: function (v) {
      return '아이 이름: ' + (_s(v.name, 40) || '(없음)') + ', 학년대: ' + (_s(v.band, 20) || '(모름)')
        + '. 오늘 집중 ' + (_n(v.focus) != null ? _n(v.focus) : '?') + '/5, 컨디션 ' + (_n(v.energy) != null ? _n(v.energy) : '?') + '/5.'
        + ' 최근에 한 걸음: ' + (_list(v.recent, 3, 60).join(' / ') || '없음') + '. 한 줄로.';
    }
  },
