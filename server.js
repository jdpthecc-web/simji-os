 ...
 * SIMJI OS 서버 (심지OS)
 * - 앱·홈페이지·구독 페이지를 서빙하고
 * - 포트원(PortOne V2) → 한국결제네트웍스(KPN) 경로로 정기결제를 처리합니다.
 *   · 빌링키 발급: 브라우저 SDK → /billing/portone-issue 로 저장
 *   · 매월 자동청구: api.portone.io 의 billing-key 결제 호출
 *   ※ 토스페이먼츠는 2026년 6월 이후 사용하지 않으며 관련 코드는 모두 제거되었습니다.
 * - AI 기능은 /ai/task 한 곳으로만 나가며, 지시문은 이 파일 안에만 있습니다.
 *
...
const AI_TASKS = {
  ...
  reading_question: {
    ...
    build: function (v) {
      ...
      return '아이 이름: ' + (_s(v.name, 40) || '(없음)')
        ...
        + (dream ? ('아이의 꿈: ' + dream + '. ') : '')
        + (_s(v.genre, 30) ? ('책 갈래: ' + _s(v.genre, 30) + '. ') : '')
  ...
  monthly_report: {
    max: 500,
    system: '너는 아동 성장 리포트 도우미. 부모에게 주는 정확히 3문장: 1) 한 달의 사실 요약 2) 기록에서 보이는 아이의 흥미·소질의 싹 한 가지(읽은 책의 갈래·감사·실천의 패턴에서, 단정하지 말고 싹이 보인다는 톤으로) 3) 다음 달 함께 해볼 한 가지. 읽은 갈래가 한두 가지에 몰려 있으면 아이가 좋아하는 갈래를 먼저 인정한 뒤 이어서 읽어볼 만한 다른 갈래를 부드럽게 권하고, 여러 갈래를 골고루 읽고 있으면 그 폭을 칭찬할 것. 평가·비교·진단 금지, 따뜻하고 구체적으로. 감사와 읽기의 꾸준함이 보이면 특히 짚어줄 것.',
    build: function (v) {
      const dream = _s(v.dream, 120);
      return '아이 ' + (_s(v.name, 40) || '아이') + '. 최근 30일: 활동 ' + (_n(v.act) || 0) + '일, 하루 완성 '
        ...
        + '. 읽은 갈래: ' + (_list(v.genres, 8, 30).join(' / ') || '기록 없음')
        + (_n(v.genreKinds) != null ? (' (총 ' + _n(v.genreKinds) + '갈래)') : '')
        + '. 작은 실천: ' + (_list(v.acts, 5, 80).join(', ') || '없음') + '.'
        + (dream ? (' 아이의 꿈: ' + dream + ' — 소질의 싹과 자연스럽게 이어지면 언급.') : '');
    }
  },

  /* 7) 動 — 오늘에 맞는 아주 작은 한 걸음 제안 */
  action_suggest: {
    max: 120,
    system: '너는 아동·청소년의 실천 코치다. 오늘 아이에게 맞는 "아주 작은 한 걸음" 하나만 제안한다. 조건: 지금 그 자리에서 1분 안에 할 수 있을 것, 준비물이 필요 없을 것, 돈이 들지 않을 것, 다칠 위험이 없을 것. 초등이면 더 쉽고 놀이처럼, 청소년이면 조금 더 실질적으로. 컨디션이 낮으면 몸을 덜 쓰는 것으로, 높으면 몸을 움직이는 것으로. 피해야 할 목록에 있는 것과 최근에 한 것은 절대 다시 말하지 말고, 매번 새로운 것을 제안할 것. 출력은 행동 문구 딱 한 줄, 15자 이내. 설명·이유·인사·마크다운·따옴표·영어·번호 금지, 한국어 평문만. 예시 형식: 물 한 컵 천천히 마시기',
    build: function (v) {
      return '아이 이름: ' + (_s(v.name, 40) || '(없음)') + ', 학년대: ' + (_s(v.band, 20) || '(모름)')
        + '. 오늘 집중 ' + (_n(v.focus) != null ? _n(v.focus) : '?') + '/5, 컨디션 ' + (_n(v.energy) != null ? _n(v.energy) : '?') + '/5.'
        + ' 이번에는 "' + (_s(v.theme, 40) || '아무거나') + '" 쪽으로 제안해 줘.'
        + ' 최근에 한 걸음: ' + (_list(v.recent, 3, 60).join(' / ') || '없음') + '.'
        + ' 피해야 할 것(이미 제안함): ' + (_list(v.avoid, 6, 60).join(' / ') || '없음') + '.'
        + ' 위와 겹치지 않는 새로운 것 한 줄로.';
    }
  },
...
async function orchMessage(a, c) {
  ...
  if (a.agent === 'parent_comm') return (await aiText('아이가 오늘 감사를 남겼어요' + (c.todayChore ? ' 그리고 집안일도 한 가지 했어요: ' + c.todayChore + '.' : '') + ' 부모가 자기 전 1분 대화를 열 부드러운 질문 한 줄을 한국어로. 집안일을 했다면 시켜서 한 일이 아니라 스스로 맡은 몫으로 인정해 주는 방향으로.', '아동-부모 연결 도우미. 따뜻하게.', 160)) || '오늘 아이가 고마워한 일을 함께 이야기해 보세요.';
